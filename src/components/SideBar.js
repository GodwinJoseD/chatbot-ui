...
export default function Sidebar({ onSettingsClick }) {
  ...
  const iconBtn = (content) => (
    <div style={{ width: 36, height: 36, borderRadius: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: t.iconColor, transition: "background 0.15s" }}>
      {content}
    </div>
  );

  const plusIconBtn = (content) => (
    <div onClick={handlePlusClick} style={{ width: 36, height: 36, borderRadius: 50, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", background: t.sidebarBg, color: t.plusIconColor, transition: "background 0.15s" }}>
      {content}
    </div>
  );

  return (
    <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", height: "100%", padding: "8px 0 16px" }}>
{panelOpen && (
        <div
  style={{
    position: "absolute",
    left: panelOpen ? 60 : -270,
    top: 0,
    width: 260,
    height: "100%",
    background: t.sidebarBg || "#1e2130",
    borderRadius: 14,
...
>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            ...
            <div style={{ opacity: 0.6, cursor: "pointer" }}>
              <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.7}><rect x="3" y="3" width="18" height="18" rx="2.5" /><line x1="9" y1="3" x2="9" y2="21" /></svg>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: "8px 12px" }}>
            <svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ opacity: 0.5, flexShrink: 0 }}><circle cx="11" cy="11" r="7" /><line x1="16.5" y1="16.5" x2="22" y2="22" /></svg>
            <input value={searchValue} onChange={(e) => setSearchValue(e.target.value)} placeholder="Search the chats" style={{ background: "none", border: "none", outline: "none", color: t.textColor || "#fff", fontSize: 13, width: "100%", opacity: searchValue ? 1 : 0.5 }} />
          </div>
            ...
            {projects.map((proj) => (
              <div key={proj.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 8px", borderRadius: 8, cursor: "pointer", fontSize: 13, opacity: 0.85 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7z" /></svg>
                  {proj.name}
                </div>
                <div style={{ opacity: 0.4, fontSize: 16, letterSpacing: 1 }}>···</div>
              </div>
            ))}
              ...
              <div key={chat.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 8px", borderRadius: 8, cursor: "pointer", fontSize: 13, background: editingChatId === chat.id ? "rgba(255,255,255,0.07)" : "transparent" }}>
                {editingChatId === chat.id ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, width: "100%" }}>
                    <input autoFocus value={editingValue} onChange={(e) => setEditingValue(e.target.value)} onBlur={() => setEditingChatId(null)} style={{ background: "rgba(255,255,255,0.08)", border: "none", outline: "none", color: t.textColor || "#fff", fontSize: 13, borderRadius: 6, padding: "3px 7px", flex: 1 }} />
                  ...
                  <>
                    <span style={{ opacity: 0.85 }}>{chat.name}</span>
                    <div onClick={() => startEditing(chat)} style={{ opacity: 0.4, fontSize: 16, letterSpacing: 1 }}>···</div>
                  </>
                )}
              </div>
            ))}
      ...
      <div style={{ paddingTop:"68px",display: "flex", flexDirection: "column", gap: "20px" }}>
        {iconBtn(<svg width="23" height="23" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.7}><rect x="3" y="3" width="18" height="18" rx="2.5" /><line x1="9" y1="3" x2="9" y2="21" /></svg>)}
        {plusIconBtn(<svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>)}
        {iconBtn(<svg width="23" height="23" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="11" cy="11" r="7" /><line x1="16.5" y1="16.5" x2="22" y2="22" /></svg>)}
      </div>

      <div onClick={onSettingsClick} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer" }}>
